...
5. Поддержка дополнительных оригинальных фич:
  ...
  - добавлены форматные символы `'r'` и `'R'` - форматирование чисел при помощи римских цифр.
...
- [marty_decimal](https://github.com/al-martyn1/marty_decimal) - поддержка чисел с плавающей точкой и произвольной размерностью.
- [marty_bigint](https://github.com/al-martyn1/marty_bigint) - поддержка целых чисел произвольного размера.
...
Для подключения следует задавать значение `1` (или любое другое ненулевое значение).
По умолчанию подключаются все библиотеки, явного подключения не требуется.
...
|:---|:---|:---|
|**unknown**, **invalid**|(std::uint32_t)(-1)|Недопустимое/неизвестное значение.|
|**none**, **romanLatin**|0x00|Пустое/отсутствующее значение.|
...
|**localeForceCustom**|0x2000|Принудительно использовать переданную локаль.|
|**localeUseSystem**|0x4000|Использовать локаль системы вместо локали текущего пользователя.|
...
**NOT_IMPLEMENTED** - пока не понятно, что делают эти преобразования. В данный момент не реализовано, но обрабатывается в форматной строке.
...
|`'H'`|`Py`/`C++`|Форматирует число в шестнадцатеричном виде. Для данного формата в альтернативной форме к отформатированномому числу добавляется суффикс `'H'` или `'h'` (с инверсией регистра).|
|`'r'`|`-`|Форматирует число с использованием римских цифр. Ноль выводится в виде строки 'nulla', бесконечность (числа больше 9999) - в виде строки 'infinitas'. Для вывода используются латинские буквы. Можно включить использование 'Unicode'-символов для римских цифр соответствующим флагом при вызове функции форматирования.|
|`'R'`|`-`|Форматирует число с использованием римских цифр. Ноль выводится в виде строки 'NULLA', бесконечность (числа больше 9999) - в виде строки 'INFINITAS'. Для вывода используются латинские буквы. Можно включить использование 'Unicode'-символов для римских цифр соответствующим флагом при вызове функции форматирования.|
