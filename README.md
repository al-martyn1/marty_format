...
|`'-'`|Символ знака `'-'` для отображения числовых значений.|
|`space`|Символ `' '` (`space`/`пробел`) для отображения числовых значений. Символ `' '` (`space`/`пробел`) используется для выравнивания по умолчанию, явное указание данного символа не требуется.|
|`'''`|Символ группировки разрядов чисел (`apos`).|
|`'_'`|Символ группировки разрядов чисел (`underscore`).|
