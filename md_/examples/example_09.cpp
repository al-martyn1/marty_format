...
void test(bool doTest=false)
{
    ...
    simpleTest(nTotal, nPassed, doTest, "3.1416"                    , ".4n"       , Args().arg( dPi) );
    std::cout << "\n";
    ...
    simpleTest(nTotal, nPassed, doTest, "3.141 593"                 , ".6_n"       , Args().arg( dPi) );
    std::cout << "\n";
    ...
    simpleTest(nTotal, nPassed, doTest, "314.159%"                  , "%"         , Args().arg( dPi) );
    std::cout << "\n";

    std::cout << "// % - проценты - число умножается на 100.\n"
                 "// Точность не задана, берётся из пользовательской локали - 2 символа\n";
    simpleTest(nTotal, nPassed, doTest, "314.16%"                   , "L%"        , Args().arg( dPi) );
    std::cout << "\n";
    ...
    simpleTest(nTotal, nPassed, doTest, "314.1593%"                 , ".4%"       , Args().arg( dPi) );
    std::cout << "\n";
    ...
    simpleTest(nTotal, nPassed, doTest, "$3.1416"                   , ".4$"       , Args().arg( dPi) );
    std::cout << "\n";
    ...
    simpleTest(nTotal, nPassed, doTest, "3.141 592 654"             , ".9'n"      , Args().arg( dPi) );
    std::cout << "\n";
    simpleTest(nTotal, nPassed, doTest, "3.142"                     , "d"         , Args().arg( dPi) );
    simpleTest(nTotal, nPassed, doTest, "3.141'592'654"             , ".9'd"      , Args().arg( dPi) );
    simpleTest(nTotal, nPassed, doTest, "3.142"                     , "f"         , Args().arg( dPi) );
    simpleTest(nTotal, nPassed, doTest, "3.141'592'654"             , ".9'f"      , Args().arg( dPi) );
    simpleTest(nTotal, nPassed, doTest, "314.159%"                  , "%"         , Args().arg( dPi) );
    simpleTest(nTotal, nPassed, doTest, "314.16%"                   , "L%"        , Args().arg( dPi) );
    simpleTest(nTotal, nPassed, doTest, "314.16"                    , "#L%"       , Args().arg( dPi) );
    simpleTest(nTotal, nPassed, doTest, "$3.14"                     , "$"         , Args().arg( dPi) );
    ...
    if (doTest)
    {
        cout << "Passed: " << nPassed << ", failed: " << (nTotal-nPassed) << " from " << nTotal << "\n";
    }
